...
const TABS = [
  { id: 'character', label: '≡ƒºì Character Accessories' },
  { id: 'environment', label: '≡ƒÅÖ∩╕Å Environment Scene' }
]
...
export default function ModelingModule({ workflowState, setWorkflowState, onChangeModule }) {
  ...
  const syncTripoHistory = useCallback(async () => {
    ...
    setHistoryMessage('Reading Tripo HistoryΓÇª')
  ...
  const generatePart = useCallback(
    async (id) => {
      ...
      setGlobalProgress({ step: 'StartingΓÇª', pct: 0 })
  ...
  const exportScene = useCallback(async () => {
    ...
    const folder = await window.api.saveFolder({ title: 'Export Scene ΓÇö Choose Folder' })
    if (!folder) return
        ...
        {tab === 'environment' && showAiFill && (
          ...
          >
            ...
            <div style={{ fontSize: 11, color: '#555b6e', lineHeight: 1.6 }}>
              Describe your scene ΓÇö DeepSeek breaks it into components and adds them to the parts list with optimised Tripo prompts ready to generate.
            </div>
            <textarea
              value={aiFillDesc}
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              >
                {aiFillBusy ? 'GeneratingΓÇª' : 'Generate Components'}
              </button>
            </div>
            {aiFillError && (
