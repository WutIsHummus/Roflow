...
const FLIPBOOK_LAYOUTS = [
  ...
  { id: 'Grid2x2', label: 'Grid 2├ù2 (4 frames)' },
  { id: 'Grid4x4', label: 'Grid 4├ù4 (16 frames)' },
  { id: 'Grid8x8', label: 'Grid 8├ù8 (64 frames)' }
]
...
function buildPromptPack(workflow, preset) {
  const references = workflow.references
    .map((reference, index) => {
      ...
      return `${index + 1}. ${reference.kind.toUpperCase()} ΓÇö ${reference.label || 'Untitled reference'}${suffix}`
    })
    .join('\n')
  ...
  return {
    ...
    handoffPrompt,
    exportText: [
      `Roblox VFX Workflow ΓÇö ${workflow.effectName}`,
      '',
      overview,
      '',
      'Analysis Prompt',
...
export default function VFXModule({ workflowState, setWorkflowState }) {
  ...
  const generateRecipe = useCallback(async () => {
    ...
    setBusy('generate')
    setNotice('Generating VFX recipe with DeepSeekΓÇª')
    ...
    setNotice(`Recipe generated ΓÇö "${r.effectName || 'effect'}" with ${nextLayers ? nextLayers.length : 0} effect parts.`)
              ...
              <div style={{ display: 'grid', gap: 12 }}>
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ability hit confirm, explosion on death, ambient fire loopΓÇª"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="0ΓÇô0.08 s flash, 0.12 s sparks, 0.3ΓÇô1.0 s smoke decayΓÇª"
                  />
                </div>
                ...
                >
                  {busy === 'generate' ? 'GeneratingΓÇª' : 'Generate VFX Recipe'}
                </button>
              </div>
            </div>
              ...
              {activeLayer ? (
                <div style={{ display: 'grid', gap: 14 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 10 }}>
                    ...
                    <div>
                      <label style={S.label}>Opacity (0ΓÇô1)</label>
                      <input
                        style={S.input}
                        type="number"
                        min="0.05"
