...
// ΓöÇΓöÇ Tripo image-to-3D: attach a reference image via CDP ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Tripo multiview: upload 2ΓÇô4 angle images via CDP ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  await win.webContents.executeJavaScript(`
    (() => {
      ...
      const el = Array.from(document.querySelectorAll('button,[role="tab"],[role="button"],li'))
        .filter(visible)
        .find(e => /multi.?view|multiple images?|σñÜΦºåΦºÆ|multi view/i.test((e.innerText || e.textContent || e.className || '')))
...
// ΓöÇΓöÇ IPC: VFX ΓÇö AI Recipe Generation (DeepSeek) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
ipcMain.handle('vfx:generateRecipe', async (_, payload) => {
  try {
    ...
    const systemPrompt = `You are a Roblox VFX expert specialising in converting Unreal Engine Niagara systems into Roblox ParticleEmitter / Beam / Trail / BillboardGui setups. You always respond with valid JSON only ΓÇö no markdown, no extra text.`
...
Return exactly this JSON structure (all fields required):
{
  ...
  "gameplayPurpose": "one sentence ΓÇö what player action triggers this effect",
...
// ΓöÇΓöÇ IPC: SFX ΓÇö AI Recipe Generation (DeepSeek) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
ipcMain.handle('sfx:generateRecipe', async (_, payload) => {
  try {
    ...
    const systemPrompt = `You are a Roblox audio designer specialising in SoundService configuration and Lua scripting. You always respond with valid JSON only ΓÇö no markdown, no extra text.`
...
For soundEffects, use only 1ΓÇô3 effects with realistic Roblox parameter values.
For layerBreakdown, include 2ΓÇô3 layers.
The luaScript must be a single JSON string ΓÇö escape all newlines as \\n and quotes as \\".`
...
// ΓöÇΓöÇ IPC: Building ΓÇö AI Recipe Generation (DeepSeek) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
ipcMain.handle('building:generateRecipe', async (_, payload) => {
  try {
    ...
    const systemPrompt = `You are a Roblox 3D environment designer and Lua scripter. You decompose building descriptions into 3D components ready for Tripo3D generation and Roblox Studio assembly. You always respond with valid JSON only ΓÇö no markdown, no extra text.`
...
Include 3ΓÇô6 components covering the most important structural parts (foundation, walls, roof, door, etc.). Order components by build priority (1 = first). The tripoPrompt must be phrased for a text-to-3D model generator ΓÇö focus on shape, style, material.`
...
// ΓöÇΓöÇ IPC: File Dialogs ΓÇö Mesh import (accessories / environment) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ IPC: GPT Image 2 clothing generation (Azure / GitHub Models) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
ipcMain.handle(
  ...
  async (event, { apiToken, prompt, inputImagePath, inputImageDataUrl } = {}) => {
    try {
      ...
      event.sender.send('clothing:progress', { step: 'Sending template to GPT ImageΓÇª', pct: 15 })
      ...
      event.sender.send('clothing:progress', { step: 'Generating clothing textureΓÇª', pct: 30 })
      ...
      event.sender.send('clothing:progress', { step: 'Downloading generated textureΓÇª', pct: 88 })
...
ipcMain.handle(
  ...
  async (
    ...
    try {
      ...
      if (isMultiview) {
        event.sender.send('modeling:progress', { step: 'Uploading multiview images to TripoΓÇª', pct: 20 })
        ...
        event.sender.send('modeling:progress', { step: 'Uploading reference image to TripoΓÇª', pct: 22 })
      ...
      if (prompt.trim()) {
        event.sender.send('modeling:progress', { step: 'Submitting prompt in browser sessionΓÇª', pct: isImageMode || isMultiview ? 30 : 24 })
        ...
        event.sender.send('modeling:progress', { step: isMultiview ? 'Triggering multiview-to-3D generationΓÇª' : 'Triggering image-to-3D generationΓÇª', pct: 30 })
...
// ΓöÇΓöÇ Blender path detection ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Mesh retopology (Blender QuadriFlow) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
ipcMain.handle('mesh:retopologyBlender', async (event, { inputPath, targetFaces = 2000, blenderPath } = {}) => {
  ...
  event.sender.send('mesh:retopologyProgress', { step: 'Starting Blender QuadriFlow retopologyΓÇª', pct: 5 })
  ...
  return new Promise((resolve) => {
    ...
    proc.stdout.on('data', (data) => {
      ...
      if (progressMatch) {
        event.sender.send('mesh:retopologyProgress', { step: 'Remeshing with QuadriFlowΓÇª', pct: parseInt(progressMatch[1]) })
      }
...
// ΓöÇΓöÇ Mesh optimization (polygon simplification + dedup/prune) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
