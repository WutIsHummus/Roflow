...
const OPTIMIZE_PRESETS = [
  { label: 'Light',  ratio: 0.75, desc: '75% ΓÇö minimal reduction, best quality' },
  { label: 'Medium', ratio: 0.50, desc: '50% ΓÇö balanced quality/performance' },
  { label: 'Heavy',  ratio: 0.25, desc: '25% ΓÇö aggressive reduction, game-ready' },
  { label: 'Ultra',  ratio: 0.10, desc: '10% ΓÇö maximum reduction, lowest poly' },
]

const RETOPO_PRESETS = [
  { label: 'Hi',   faces: 5000, desc: '5 000 faces ΓÇö high detail, closer to source' },
  { label: 'Mid',  faces: 2000, desc: '2 000 faces ΓÇö balanced quad topology' },
  { label: 'Lo',   faces: 800,  desc: '800 faces ΓÇö game-ready low poly' },
  { label: 'Tiny', faces: 300,  desc: '300 faces ΓÇö minimal quad mesh' },
]

const ATTACH_POINTS = [
  // Head
  { id: 'HatAttachment',       label: '≡ƒÄ⌐ Hat (Top of Head)' },
  { id: 'HairAttachment',      label: '≡ƒÆç Hair (Back of Head)' },
  { id: 'FaceCenterAttachment',label: '≡ƒæü∩╕Å Face Center' },
  { id: 'FaceFrontAttachment', label: '≡ƒÑ╕ Face Front' },
  // Upper Torso
  { id: 'NeckAttachment',      label: '≡ƒöù Neck' },
  { id: 'BodyFrontAttachment', label: '≡ƒª║ Body Front' },
  { id: 'BodyBackAttachment',  label: '≡ƒÄÆ Body Back' },
  { id: 'LeftCollarAttachment', label: 'ΓùÇ Left Collar' },
  { id: 'RightCollarAttachment', label: 'Γû╢ Right Collar' },
  // Lower Torso
  { id: 'WaistCenterAttachment', label: '≡ƒö░ Waist Center' },
  { id: 'WaistFrontAttachment',  label: 'Γ¼å Waist Front' },
  { id: 'WaistBackAttachment',   label: 'Γ¼ç Waist Back' },
  // Arms
  { id: 'LeftShoulderAttachment',  label: 'ΓùÇ Left Shoulder' },
  { id: 'RightShoulderAttachment', label: 'Γû╢ Right Shoulder' },
  // Hands
  { id: 'LeftGripAttachment',  label: 'Γ£ï Left Grip' },
  { id: 'RightGripAttachment', label: '≡ƒñÜ Right Grip' },
  // Feet
  { id: 'LeftFootAttachment',  label: '≡ƒæƒ Left Foot' },
  { id: 'RightFootAttachment', label: '≡ƒæƒ Right Foot' },
]
      ...
      <div style={{ padding: '14px 16px', borderBottom: '1px solid #1e2330', flexShrink: 0 }}>
        ...
        {showTripoBrowser && (
          <div style={{ marginTop: 10 }}>
            ...
            >
              <span>Γ¼í Browse Tripo History</span>
            ...
            {showAssets && (
              ...
              >
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
                  ...
                  >
                    {assetRefreshState === 'loading' ? 'SyncingΓÇª' : 'Sync Tripo History'}
                  </button>
                  {assetRefreshMessage && (
                    <span
                      style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px' }}>
        {parts.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '36px 16px', color: '#3e4455' }}>
            <div style={{ fontSize: 28, marginBottom: 10, opacity: 0.5 }}>ΓÜÖ</div>
...
function PartCard({ part, index, onRemove, onDuplicate, onGenerate, onPartChange, onOptimize, onRetopo, showAttachPoint }) {
  ...
  const STATUS = {
    ...
    generating: { color: '#f59e0b', label: 'GeneratingΓÇª' },
    done:       { color: '#4ade80', label: 'Done Γ£ô' },
  ...
  return (
    <div style={{
      ...
      <textarea
        ...
        placeholder={imageMode ? 'Optional prompt for image-to-3DΓÇª' : 'Describe this partΓÇª'}
        rows={3}
        spellCheck={false}
        autoCorrect="off"
        autoCapitalize="off"
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
        ...
        >
          {busy ? <><SmallSpinIcon />GeneratingΓÇª</> : part.status === 'done' ? 'Γå║ Regenerate' : 'ΓÜí Generate Part'}
        </button>
        {part.status === 'done' && part.outputPath && (
          <div style={{ display: 'flex', gap: 4, alignItems: 'stretch' }}>
            ...
            >
              {part.optimizeState === 'optimizing' ? <><SmallSpinIcon />OptimizingΓÇª</>
                : part.optimizeState === 'done' ? `Γ£ô -${part.optimizeSaved ?? 0}%`
                : 'Γ£ª Optimize'}
            </button>
          </div>
        )}
      ...
      {part.optimizeState === 'error' && part.optimizeError && (
        <div style={{ marginTop: 5, fontSize: 10, color: '#fca5a5', lineHeight: 1.5 }}>
          ΓÜá {part.optimizeError}
        </div>
      )}

      {/* Retopo row ΓÇö shown when a mesh is ready */}
      {part.status === 'done' && part.outputPath && (
        <div style={{ display: 'flex', gap: 4, alignItems: 'stretch', marginTop: 6 }}>
          ...
          <button
            ...
            disabled={part.retopoState === 'retopoing'}
            title={`Blender QuadriFlow ΓÇö ${RETOPO_PRESETS[retopoFaceIdx].desc}`}
            style={{
              flex: 1,
              padding: '9px 11px',
              borderRadius: 8,
          ...
          >
            {part.retopoState === 'retopoing' ? <><SmallSpinIcon />RetopoingΓÇª</>
              : part.retopoState === 'done' ? 'Γ£ô Retopo'
              : 'Γ¼í Retopo'}
          </button>
        </div>
      )}
