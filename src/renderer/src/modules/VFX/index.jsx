...
const SOURCE_MODES = [
  ...
  { id: 'niagara-rebuild', label: 'Niagara → Roblox' }
]
...
const FLIPBOOK_LAYOUT_OPTIONS = [
  { id: 'None', label: 'None' },
  { id: 'Grid2x2', label: 'Grid 2×2' },
  { id: 'Grid4x4', label: 'Grid 4×4' },
  { id: 'Grid8x8', label: 'Grid 8×8' }
]
...
function buildVfxTextureImagePrompt(workflow, layer) {
  ...
  return [
    `Roblox particle texture sprite for "${effectName}" — ${layerRole}.`,
...
function buildLayerRobloxMapping(layer) {
  ...
  const textureValue =
    textureSource === 'image' && layer.textureImagePath
      ? `rbxassetid://… (${fileNameFromPath(layer.textureImagePath)})`
...
export default function VFXModule({ workflowState, setWorkflowState }) {
  ...
  const generateLayerTextureWithElevenLabs = useCallback(
    async (layer) => {
      ...
      setProgress({ step: 'Checking ElevenLabs session…', pct: 4 })
      ...
      if (!session?.connected) {
        ...
        setBusy('')
        setProgress(null)
        setNotice('ElevenLabs session not connected. Prompt copied — log in, then try again.')
        return
      }

      setProgress({ step: 'Generating texture in ElevenLabs…', pct: 12 })
  ...
  const generateParticleLogic = useCallback(async () => {
    ...
    setBusy('generate')
    setProgress({ step: 'Preparing request…', pct: 4 })
    setNotice('')
  ...
  const generateRecipe = useCallback(async () => {
    ...
    setWorkflow((prev) => ({
      ...
      generatedNotes: r.implementationNotes || prev.generatedNotes,
      ...
      <div style={S.header}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 20, alignItems: 'flex-start' }}>
          <div>
            ...
            <p style={S.subtitle}>
              Describe an effect or paste Niagara reference data — DeepSeek V4 translates it into Roblox-ready particle
              layers.
            </p>
          </div>
              ...
              <div style={{ display: 'grid', gap: 12 }}>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={
                      workflow.sourceMode === 'niagara-rebuild'
                        ? 'Optional context: readability goals, mobile constraints, which Niagara modules matter most…'
                ...
                <button
                  style={{ ...S.subtleButton, width: '100%' }}
                  disabled={busy === 'generate'}
                  onClick={generateRecipe}
                >
                  Generate VFX Recipe (Niagara JSON)
                </button>
                {progress && (
                ...
                <div style={{ display: 'grid', gap: 12 }}>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Spawn rate, lifetime, velocity, color over life, curl/noise, module stack…"
                    />
                  </div>
                  <div>
                    ...
                    {(activeLayer.textureSource || 'shape') === 'shape' ? (
                      ...
                      <div style={{ display: 'grid', gap: 10 }}>
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                          ...
                          >
                            {busy === `elevenlabs-${activeLayer.id}` ? 'Generating…' : 'Generate with ElevenLabs'}
                          </button>
                        ...
                        {workflow.references.some((reference) => reference.kind === 'image') ? (
                          <LayerSelectField
                            ...
                            value=""
                            options={[
                              { id: '', label: 'Choose reference image…' },
                              ...workflow.references
                  ...
                  <LayerSection title="Advanced" defaultOpen={false}>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="Roblox setup notes…"
                      />
                    </div>
                  </LayerSection>
